...
auto constexpr Gravity = 0.09375;
auto constexpr CollisionWidthRadius = 12;
auto constexpr CollisionHeightRadius = 16;

Nedrysoft::Chopper::Chopper(int subType, float x, float y, bool rememberState, bool mirrored, bool flipped) :
        Nedrysoft::Object("enemy-chopper", subType, x, y, rememberState, mirrored, flipped) {

    reset();
}

auto Nedrysoft::Chopper::initialise() -> void {

}

auto Nedrysoft::Chopper::update() -> void {

}

auto Nedrysoft::Chopper::render(Camera *camera, ObjectRenderPriority priority) -> void {
...
auto Nedrysoft::Chopper::update(TileMap *tileMap, Camera *camera, class Sonic *sonic) -> bool {
    int width;
    int height;
    Vector origin;
...
auto Nedrysoft::Chopper::spawn(Camera *camera) -> bool {
    if (!m_alive) {
        return false;
    }

    auto animations = Nedrysoft::ChopperFactory::getInstance()->animations();

    assert(animations);

    animations->start("fast", &m_animation);

    return true;
}

auto Nedrysoft::Chopper::renderCollision(Camera *camera) -> void {
    if (!m_alive) {
        return;
    }
...
auto Nedrysoft::Chopper::reset() -> void {
    Object::reset();
